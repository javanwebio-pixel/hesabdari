...
export const NewPaymentPage: React.FC<NewPaymentPageProps> = ({ onNavigate, addTreasuryDoc, showToast, bankAccounts, cashDesks, parties }) => {
    ...
    const suppliers = parties.filter(p => p.code.startsWith('2'));
    ...
    const validate = () => {
        ...
        if (!selectedParty) newErrors.partyName = '"پرداخت به" الزامی است.';
        if (!sourceId) newErrors.sourceId = 'حساب مبدا الزامی است.';
    ...
    const handleSave = () => {
        ...
        const newDoc: Omit<TreasuryDoc, 'id' | 'docNumber'> = {
            ...
            description: formData.description || `پرداخت به ${selectedParty.name}`,
            paymentMethod,
            type: 'پرداخت',
            status: 'نهایی',
            bankAccountId: source === 'bank' ? sourceId : undefined,
            cashDeskId: source === 'cash' ? sourceId : undefined,
        ...
        showToast('اعلامیه پرداخت با موفقیت ثبت شد.');
        onNavigate('treasury-dashboard');
    };

    const renderPaymentMethodFields = () => {
        // ... (render logic for check, transfer, etc. fields)
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800 dark:text-white">اعلامیه پرداخت جدید</h1>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    ...
                    <div>
                        <label className="label-form">پرداخت به</label>
                        <SearchableSelector
                            items={suppliers}
                ...
                <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                    <label className="label-form mb-2">برداشت از:</label>
                    <div className="flex bg-gray-200 dark:bg-gray-700 rounded-lg p-1 mb-2">
                        <button onClick={() => { setSource('bank'); setSourceId(''); }} className={`w-1/2 p-2 rounded-md text-sm font-semibold transition-colors ${source === 'bank' ? 'bg-white dark:bg-gray-900 text-primary shadow' : 'text-gray-600 dark:text-gray-300'}`}>بانک</button>
                        <button onClick={() => { setSource('cash'); setSourceId(''); }} className={`w-1/2 p-2 rounded-md text-sm font-semibold transition-colors ${source === 'cash' ? 'bg-white dark:bg-gray-900 text-primary shadow' : 'text-gray-600 dark:text-gray-300'}`}>صندوق</button>
                    </div>
                    ...
                    {errors.sourceId && <p className="text-xs text-danger mt-1">{errors.sourceId}</p>}
                </div>

                <div className="mb-6">
                    <label className="label-form mb-2">روش پرداخت</label>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {(['حواله', 'چک', 'نقد'] as PaymentMethod[]).map(method => (
                           method !== 'کارتخوان' && <button key={method} onClick={() => setPaymentMethod(method)} className={`flex items-center justify-center gap-2 p-3 rounded-lg border-2 transition-colors ${paymentMethod === method ? 'border-primary bg-primary-50 dark:bg-gray-700' : 'border-gray-200 dark:border-gray-600 hover:border-primary-200'}`}>
                                {method === 'نقد' && <IconWallet className="w-5 h-5" />} {method === 'چک' && <IconFileText className="w-5 h-5" />}
                                {method === 'حواله' && <IconBuildingBank className="w-5 h-5" />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <div><label className="label-form">مبلغ پرداختی (ریال)</label><input type="number" name="amount" value={formData.amount} onChange={handleChange} placeholder="مبلغ را وارد کنید" className="input-field" />{errors.amount && <p className="text-xs text-danger mt-1">{errors.amount}</p>}</div>
                    <div className="md:col-span-2"><label className="label-form">شرح</label><input type="text" name="description" value={formData.description} onChange={handleChange} placeholder="توضیحات مربوط به پرداخت..." className="input-field" /></div>
                </div>
